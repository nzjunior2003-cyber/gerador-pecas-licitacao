    ...
    <div className="space-y-6">
      <Section title="Identificação do Documento">
        <div className="grid md:grid-cols-3 gap-6">
          <Field label="Número do ETP" required><input type="text" name="numero" value={data.numero} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md" /></Field>
          <Field label="Ano" required><input type="text" name="ano" value={data.ano} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md" /></Field>
          <Field label="PAE nº" required><input type="text" name="pae" value={data.pae} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md" /></Field>
        </div>
      </Section>
      
      <Section title="1. Descrição da Necessidade">
        <Field label="1.1. Qual a necessidade a ser atendida?" required>
          <textarea name="necessidade" value={data.necessidade} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24" />
        </Field>
      </Section>
      ...
      <Section title="2. Levantamento de Mercado">
        <Field label="2.1. Onde foram pesquisadas as possíveis soluções?" required>
            {['Consulta a fornecedores', 'Internet', 'Contratações similares', 'Audiência pública', 'Outro'].map(val => (
                <div key={val} className="flex items-center"><input type="checkbox" name="fontesPesquisa" value={val} checked={data.fontesPesquisa.includes(val)} onChange={handleCheckboxChange} className="mr-2"/>{val}</div>
            ))}
            {data.fontesPesquisa.includes('Outro') && <input type="text" name="fonteOutro" value={data.fonteOutro} onChange={handleChange} placeholder="Especifique" className="w-full mt-2 p-2 border border-gray-300 rounded-md"/>}
        </Field>
        <Field label="2.2. Justificativa técnica e econômica para a escolha da melhor solução" required><textarea name="justificativaTecnica" value={data.justificativaTecnica} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24" /></Field>
      ...
      <Section title="3. Requisitos da Contratação">
            ...
            <div className="flex flex-col gap-y-2">
                {[
                    ...
                    <div key={opt.val} className="flex items-start">
                        ...
                        />
                        <label htmlFor={`tipoObjeto-${opt.val}`}>{opt.label}</label>
                    </div>
                ))}
            </div>
        </Field>
        ...
        <Field label="3.4. Qual a vigência do contrato?" required>
            ...
            {data.vigencia === 'outro' && (
                <div className="flex items-center gap-2 mt-2 p-2 border rounded-md bg-white">
                    <input type="number" name="vigenciaOutroNum" value={data.vigenciaOutroNum} onChange={handleChange} className="p-2 border border-gray-300 rounded-md w-1/3" placeholder="Nº"/>
                    <select name="vigenciaOutroTipo" value={data.vigenciaOutroTipo} onChange={handleChange} className="p-2 border border-gray-300 rounded-md w-2/3">
        ...
        {data.transicao === 'sim' && (
          <div className='grid md:grid-cols-2 gap-4 mt-2'>
            <input name="transicaoContrato" value={data.transicaoContrato} onChange={handleChange} placeholder="Nº do Contrato" className="w-full p-2 border border-gray-300 rounded-md"/>
            <input name="transicaoPrazo" value={data.transicaoPrazo} onChange={handleChange} placeholder="Prazo" className="w-full p-2 border border-gray-300 rounded-md"/>
          </div>
        )}
        <div className="mt-4">
            <h3 className="text-gray-700 font-semibold mb-2">3.7. Padrão Mínimo de Qualidade</h3>
            {data.padraoQualidade.map((item, index) => (
                <div key={item.id} className="p-4 border rounded-md mb-2 bg-white">
                    <div className="flex justify-between items-center mb-2">
                        <h4 className="font-semibold">Item de Qualidade {index + 1}</h4>
                        <button onClick={() => removeQualidadeItem(item.id)} className="text-red-500 hover:text-red-700 font-bold">Remover</button>
                    </div>
                    <textarea value={item.descricao} onChange={(e) => handleQualidadeChange(item.id, e.target.value)} className="w-full p-2 border border-gray-300 rounded-md h-20" placeholder="Descrição detalhada"/>
                </div>
            ))}
        ...
        <Field label="3.8. Critérios de Sustentabilidade">
            <div className="flex items-center">
                ...
                />
                <label htmlFor="sustentabilidade-check">Apresenta critérios de sustentabilidade</label>
            </div>
            {data.sustentabilidade.includes('Critérios de sustentabilidade.') && (
                <textarea 
                    ...
                    onChange={handleChange} 
                    className="w-full p-2 border border-gray-300 rounded-md h-20 mt-2"
            ...
            <div className="flex flex-col gap-y-2">
                {[
                    ...
                    <div key={opt.val} className="flex items-center">
                        ...
                        <label htmlFor={`prioridadeLei-${opt.val}`}>{opt.label}</label>
                    </div>
                ))}
            </div>
       ...
       <Section title="4. Descrição da Solução como um todo">
         <Field label="4.1. Solução da Contratação" required><textarea name="solucaoContratacao" value={data.solucaoContratacao} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
         <Field label="4.2. Garantia Contratual">
            ...
            {data.garantiaContratual === 'outro' && (
                <div className="flex items-center gap-2 mt-2 p-2 border rounded-md bg-white">
                    <input type="number" name="garantiaOutroNum" value={data.garantiaOutroNum} onChange={handleChange} className="p-2 border border-gray-300 rounded-md w-1/3" placeholder="Nº"/>
                    <select name="garantiaOutroTipo" value={data.garantiaOutroTipo} onChange={handleChange} className="p-2 border border-gray-300 rounded-md w-2/3">
                        <option value="dias">dias</option>
                        <option value="meses">meses</option>
                        <option value="anos">anos</option>
                    </select>
         ...
         {data.assistenciaTecnica === 'sim' && <Field label="Justificativa Assistência"><input name="assistenciaJust" value={data.assistenciaJust} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded-md"/></Field>}
         ...
         {data.manutencao === 'sim' && <Field label="Descrição Manutenção"><input name="manutencaoDesc" value={data.manutencaoDesc} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded-md"/></Field>}
       </Section>
      ...
      <Section title="5. Dimensionamento do Objeto">
        <Field label="5.1. Método para estimar as quantidades" required>
            {['Memória de cálculo', 'Histórico de consumo', 'Outro'].map(val => (
                <div key={val} className="flex items-center"><input type="checkbox" name="metodoQuantitativo" value={val} checked={data.metodoQuantitativo.includes(val)} onChange={handleCheckboxChange} className="mr-2"/>{val}</div>
            ))}
            {data.metodoQuantitativo.includes('Outro') && <input type="text" name="metodoOutro" value={data.metodoOutro} onChange={handleChange} placeholder="Especifique" className="w-full mt-2 p-2 border border-gray-300 rounded-md"/>}
        </Field>
        <Field label="5.2. Descrição do Quantitativo" required><textarea name="descricaoQuantitativo" value={data.descricaoQuantitativo} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
        <div className="mt-4">
            <h3 className="text-gray-700 font-semibold mb-2">5.3. Especificação</h3>
            {data.itens.map((item, index) => (
              <div key={item.id} className="p-4 border rounded-md mb-2 bg-white">
                ...
                <Field label="Descrição" required><textarea value={item.descricao} onChange={e => handleItemChange(item.id, 'descricao', e.target.value)} required className="w-full p-2 border border-gray-300 rounded-md h-20"/></Field>
                <div className="grid md:grid-cols-3 gap-4">
                    <Field label="Unidade" required><input type="text" value={item.unidade} onChange={e => handleItemChange(item.id, 'unidade', e.target.value)} required className="w-full p-2 border border-gray-300 rounded-md"/></Field>
                    <Field label="Quantidade" required><input type="number" value={item.quantidade} onChange={e => handleItemChange(item.id, 'quantidade', parseFloat(e.target.value) || 0)} required className="w-full p-2 border border-gray-300 rounded-md"/></Field>
                    <Field label="Valor Unitário (R$)" required><input type="number" value={item.valorUnitario} onChange={e => handleItemChange(item.id, 'valorUnitario', parseFloat(e.target.value) || 0)} required className="w-full p-2 border border-gray-300 rounded-md"/></Field>
                </div>
              </div>
            ))}
                    ...
                    <tbody>
                        {data.itens.length > 0 ? data.itens.map((item, index) => (
                            <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                                <th scope="row" className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap">{index + 1}</th>
                          ...
                          <tr className="bg-white border-b"><td colSpan={5} className="px-4 py-4 text-center text-gray-500">Nenhum item adicionado.</td></tr>
                        )}
                    </tbody>
                    <tfoot>
      ...
      <Section title="7. Justificativa para o Parcelamento ou Não">
          <Field label="7.1. Justificativa" required><textarea name="justificativaParcelamento" value={data.justificativaParcelamento} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
      </Section>
      
      <Section title="8. Resultados Pretendidos">
          <Field label="8.1. Resultados" required><textarea name="resultadosPretendidos" value={data.resultadosPretendidos} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
      </Section>
      
      <Section title="9. Providências a serem Adotadas">
          <Field label="9.1. Providências" required><textarea name="providencias" value={data.providencias} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
      </Section>
      
      <Section title="10. Possíveis Impactos Ambientais">
          <Field label="10.1. Impactos" required><textarea name="impactosAmbientais" value={data.impactosAmbientais} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
      </Section>

      <Section title="11. Contratações Correlatas e/ou Interdependentes">
          <Field label="11.1. Contratações" required><textarea name="contratacoesCorrelatas" value={data.contratacoesCorrelatas} onChange={handleChange} required className="w-full p-2 border border-gray-300 rounded-md h-24"/></Field>
      </Section>
